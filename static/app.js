...
var questionTypeDisplay = document.getElementById("questionTypeDisplay")
var fileCreationLinkContainer = document.querySelector(".fileCreationLinkContainer")
...
function returnToStart() {
  ...
  fileCreationLinkContainer.classList.remove("hide")
  questionNumber = 0
...
function copyQuestionType(){
  questionType = 0
  questionTypeDisplay.classList.remove("hide")
  questionTypeDisplay.innerHTML = "Copy the answer given below:"
  questionContainer.classList.remove("hide")
  ...
  prompt.innerHTML = "Prompt: " + aArray[questionNumber];
...
function writeFromMemory(){
  questionContainer.classList.remove("hide")
  questionTypeDisplay.classList.add("hide")
  questionType = 4
  answerInput.removeAttribute("readonly")
  answerInput.classList.remove("hide")
  answerChangeDisplay.classList.add("hide")
  correctAnswerDisplay.classList.add("hide")
  question.innerHTML = "There is no help for this last one. Try to remember it on your own!"
  prompt.innerHTML = "Prompt: " + aArray[questionNumber];
  currentQuestion = qArray[questionNumber]
  //   question.innerHTML = shuffledQuestions[questionNumber];
  //   prompt.innerHTML = shuffledAnswers[questionNumber];   //use these for unordered questions
...
function shuffleQuestionType(){
  questionTypeDisplay.classList.remove("hide")
  questionTypeDisplay.innerHTML = "Put the shuffled words into the correct order: "
  questionContainer.classList.remove("hide")
  questionType = 3
  answerInput.removeAttribute("readonly")
  answerInput.classList.remove("hide")
  ...
  question.innerHTML = "Shuffled answer: " + currentShuffledQuestion;
  prompt.innerHTML = "Prompt: " + currentAnswer;
...
function gapFillQuestion(paramaterType){
  ...
  promptGapFill.innerHTML = "Prompt: " + currentAnswer;
  var currentQuestionWords = currentQuestion.split(" ")
  for (var i=0;i<currentQuestionWords.length;i++){   //change back to -1 after currentQUestionwords.length
...
function fileInputed(e){
  fileCreationLinkContainer.classList.add("hide")
  if (e!=false){
    isFileInputed = true;
    fileTarget = e.target
